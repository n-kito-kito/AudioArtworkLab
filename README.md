...
中央のキャンバスには、デフォルトのWebGL波形を含め、画像・円・波形・テキストをレイヤーとして配置できます。デフォルト波形も前後移動と削除が可能です。各デザインレイヤーはドラッグ移動、拡大縮小、回転、色、透明度、Blur、Contrast、重なり順を調整でき、Volume / Bass / Mid / Treble / Beatへ反応させられます。画像はローカルファイルから読み込みでき、すべてのレイヤーを含むPNGを書き出せます。

右上の `Controls` と `Inspector` から左右のパネルを個別に開閉できます。デスクトップでは閉じたパネルの領域までキャンバスが広がり、モバイルでは片側ずつスライド表示されます。
