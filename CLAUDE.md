...
**D10 以降（2026-07-25〜）は `PRD.md` §12 が正。** 要点: 境界は粒子密度で描く（D10）/ モノクロ既定・Palette Map 温存（D13）/ 恣意性の排除はジェネレーターに適用、Effect は調整可（D14）/ Design タブ撤去（D15）/ **1 表現 = 1 見え方。Renderer 選択は廃止し内部部品化**（D16）/ **楽器のモデル: 境界は「生成と閾値」。範囲・感度は焼き込み内部定数。ズームは開発用で主 UI に置かない**（D17）。
