...
**D10 以降（2026-07-25〜）は `PRD.md` §12 が正。** 要点: 境界は粒子密度で描く（D10）/ モノクロ既定・Palette Map 温存（D13）/ 恣意性の排除はジェネレーターに適用、Effect は調整可（D14）/ Design タブ撤去（D15）/ **1 表現 = 1 見え方。Renderer 選択は廃止し内部部品化**（D16）。
