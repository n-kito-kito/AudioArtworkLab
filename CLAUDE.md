...
**目的・非目標・品質基準・決定履歴は [PRD.md](./PRD.md) が唯一の正。作業前に必ず読むこと。**
制作思想の一次資料は [PRODUCT_PHILOSOPHY.md](./PRODUCT_PHILOSOPHY.md)（判断の背景を知りたいとき読む）。
...
**D17 恣意性の排除** — 範囲・感度は焼き込み内部定数、UI に出さない。ズームは開発用 /
...
**D24 目的の再定義** — **音を起点に映像を生み出すツール**。O/S 自身の道具（配布物ではない）で、映像は押しつけない触媒として Healthy Psychedelia を目に見える形にする。VJ 中に触れるのは表現切替・テーマ・エフェクト+「音のどこにどれだけ反応させるか」。像そのものは音だけが決める /
**D25 共通原則と表現別方針の分離** — 全体を縛るのは非目標と恣意性の排除のみ。サイマティクス固有の方針（境界・状態変化・板・対称性）を全体方針として書かない。表現ごとに持つ機能を宣言（サイマティクスは色テーマのみ・**奥行きなし**）。動画書き出しは MP4 /
**D26 画角は長方形の板** — 7 比率。切り取り・余白ではなく板ごと長方形（場は物理座標で評価・シム解像度も比率割り・向きは 180° 単位）。反応の調整は Bass/Mid/Treble → 砂の活発さの 3 スライダー（Preset v9）/
**D27 思想の保存と語の整理** — 一次資料は PRODUCT_PHILOSOPHY.md。**「楽器」という語は使わない**。表現は数を競わないが数個に限定もしない。配布は保留だが完成度は配布水準を目指す /
**D28 Simple Rules, Complex Behaviors（2026-07-30）** — 生成の構造は「音のパラメータ × 表現のパラメータを小さなルールで結び、重なりが表現を生む」（音楽 → 特徴抽出 → 接続 → Expression → Effect。§3.2/§5 更新。3 層写像はサイマティクス固有へ）/
**D29〜D34 プリズム光（2026-07-30）** — 視覚要素と音反応を分解して検証（Trigger→Texture→Variation→Composite が必須工程）/ 発光イベント時に特徴を確定し以後は光量推移のみ（Z 移動禁止）/ 決定論 Seed + 履歴回避 / 複数レイヤーは 1 つの Prismatic Burst / Study・Lab は開発用例外で最終的に 1 表現へ統合 / 品質は PRD §8-2 の明文条件で判定
...
| `PRODUCT_PHILOSOPHY.md` | 制作思想の一次資料（2026-07-27・原文のまま）。創造性の所在・制約の理由・判断の問い |
