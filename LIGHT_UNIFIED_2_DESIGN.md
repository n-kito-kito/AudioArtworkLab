...
## 0. 最上位原則: Simple Rules, Complex Behavior

Light Unified 2は、Spatial / Reactive / Lab2 / Driftという完成形を部品として切り替える仕組みではなく、
**単純な光の規則を独立して組み替え、その相互作用から複雑な光を生む仕組み**として設計する。

最初から「中央にCoreがある」「Cross Rayがある」「この見た目はLab2である」と固定しない。
既存表現は完成見本・ドナー・Recoveryとして保持するが、その構造を新しい表現すべてへ強制しない。

| 規則層 | 1つずつ扱う内容 | 組み合わせで起きること |
|---|---|---|
| Emission | 強さ、個数、発生位置、タイミング、寿命、常在性 | 瞬発、連続発光、疎密 |
| Material | 素材番号、切り取り位置、形、サイズ | 点、膜、ガラス片、光片の差 |
| Placement / Overlap | 位置、奥行き、分散、向き、加算合成 | 重なった場所の白熱、空間的な塊 |
| Transform / Motion | 移動、漂い、歪み、ねじれ、軌跡 | 素材を保ったまま複雑な運動へ変化 |
| Effect | Bloom、分光、残像、Feedback、質感 | 光学的な仕上げ |

各規則は1つの視覚的意味だけを持ち、単独で最小・中央・最大を検証できるようにする。
複数の規則を同時に追加して完成形へ近づけるのではなく、1規則ずつ成立させた後に組み合わせる。

旧Spatialで、同じ光素材を変形・配置し、加算合成で重ねるだけでも白熱や複雑な形が生まれた構造は、
この原則に近い重要なドナーである。必要に応じてCymaticsの図形を配置・変形規則として接続できるようにし、
「光 + Cymatics」のようなCompositionも将来の組み合わせとして残す。

現在の`Volume → Global Intensity`と`Bass → Scale`は、1入力が1つの意味へ接続された原子的な規則として妥当である。
今後も、音と表現の接続は一度に1本ずつ追加・確認する。

### 既存の名称と要素の位置づけ

- Spatial / Reactive / Lab2 / Drift: 完成見本、Recovery、推奨開始地点。システムの基礎単位ではない
- Style Preset: 規則群の推奨値へ戻る開始地点。独立Rendererや閉じた表現ではない
- Core / Fragment / Membrane / Haze: 再利用可能な素材・要素。全状態への必須パーツではない
- Cross Ray: 旧Lab2のRecoveryでは独立Elementとして保存するが、Unified側の必須構造にはしない
- Effect: 素材や発生規則から分離し、最後に重ねる

Cross Rayが汎用的なRayの配置・重なりから自然に作れるか、それでも専用Elementが必要かは、
単純規則を組み合わせた結果を見てから決める。Cymaticsとの具体的な接続方法も未確定事項として残す。
...
第2接続は`Bass（低域）→ Scale（光素材の広がり）`とする。基準0.62を維持し、低域が最大でも
0.80までの上乗せに留める。Intensityとは異なる「形の面積」の反応を確認する段階であり、
位置・色・Cross Ray・Recoveryには影響させない。
