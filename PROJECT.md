...
AALが提供する最小単位は、完成した見た目や名前付きの表現ではなく、**1つの意味だけを持つ単純な規則**とする。
...
単純な規則を独立して増やし、その組み合わせから複雑で予想外の振る舞いを生む。
Core、Cross、Spatial、Lab2のような名称や完成形を、すべての表現が従う基礎構造にはしない。
それらは規則の組み合わせから生まれる結果、開始地点、または再利用可能な任意要素として扱う。

1つのパラメーターへ複数の責務を持たせず、各規則は単独で理解・検証できるようにする。
Audioは規則へ値を供給するが、見た目や組み合わせ方そのものは決定しない。
