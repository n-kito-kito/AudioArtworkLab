...
- [ ] G4 2 つ目の表現を追加し、表現の切り替え（トランジション）を作る
  ...
  - [ ] Light Element Lab 2 の音接続 Step 5 — 音色の持続値 → H のイベント的切替と
        音由来の seed。**恒久方針のとおり、追加する光もフェードさせずティック単位で
        出し入れする。** 配線はすべて `opticsAudioDrive.ts` の中だけで進める。
        `huePhase` は未配線のまま開発つまみを維持している。
