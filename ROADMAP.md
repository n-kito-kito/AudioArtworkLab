...
- [ ] 表現ごとの機能宣言に UI を合わせる — サイマティクスから奥行きスライダーを外す（PRD D25。プリセットの版上げを伴う）
- [ ] UI を理解できるところまで削る — 意味を説明できない要素（フッター表記・品質モニター・Effect の詳細パラメータ等）を一旦非表示にする（削除はしない。MTG 2026-07-27）
- [ ] 動画書き出しを WebM から MP4 へ移行する（PRD D25。方式調査から）
- [ ] 区間を指定して書き出せるようにする（ここからここまでを録画。MTG 2026-07-27）
- [ ] 画角 — 正方形固定をやめ、アスペクト比を選べるようにする（板と画角の関係の設計が先。MTG 2026-07-27）
- [ ] ツールバーの整理 — PNG 書き出しが最も目立つボタンで良いか見直す（MTG 2026-07-27）
- [ ] 反応マッピングの演奏 UI — 音のどこに・どれだけ反応させるかをリアルタイムで触れるようにする（PRD D24。設計合意が先）
