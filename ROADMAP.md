...
- [ ] G4 2 つ目の表現を追加し、表現の切り替え（トランジション）を作る
  ...
  - [x] Spatial Study の次段階 — 4 つの手掛かりを追加し、**どれも独立に切って比較できる**。
        ① 床のグリッド: 自発光させず **Core の光にだけ照らされる**面（無音＝黒画面を守る。
        光源は Core の位置 × 色 × いまの明るさをユニフォーム配列で渡し、線の縁は
        fwidth を使わず距離あたりのピクセル幅で AA・遠方は指数フェードでモアレを消す）。
        ② カメラのゆっくりした平行移動: 互いに素な 2 周期の正弦で動き、視線は奥の 1 点へ
        固定して視差を出す。**音へは結ばず**、振幅は位置生成の余白内に収めて端の Core を
        切らない。③ 帯域ごとの大きさ差（Bass 大・Treble 小。sizeAmount で運転）。
        ④ 軌跡の太さを帯域へ結ぶ（Bass 太い帯・Treble 細い筋。Trail width つまみ）。
        速度写像と合わせて「重いものは大きくゆっくり太く、軽いものは小さく速く細く」で統一
  - [ ] Spatial Study の評価 — 4 つの手掛かりを 1 つずつ切って奥行きの読みやすさを実楽曲で
        比べ、効かないものは外す。床の高さ・格子間隔・光の届く半径、カメラの振幅・周期の
        質感はその際に追い込む（定数はすべて `SPATIAL_STUDY` / `LIGHT_MAPPING`）
