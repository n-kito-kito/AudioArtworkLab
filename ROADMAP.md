...
- [ ] フルスクリーン出力 — 第 2 画面（プロジェクター/LED）へ絵だけを全画面で出す。VJ 実践の前提
- [ ] UI の主従の再編 — Expression > 反応の調整 > Effect の序列を画面に反映する（PRD §3.2。Effect stack を二次的な置き場へ）
- [ ] 表に出す Effect の絞り込み — 「説明できるものだけ残す」基準でユーザーが○×を付け、それ以外は隠す
