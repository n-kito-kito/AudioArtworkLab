...
- [ ] UI の主従の再編 — Expression > 反応の調整 > Effect の序列を画面に反映する（PRD §3.2。試作 3 案を `?ui=1|2|3` で並置済み・選定待ち）
- [ ] キーボードショートカット — 表現・テーマ・Effect の切替や反応の調整をキーで直感的に操作する。制約のあるツールだからこそ割当を固定できる（UI 再編の選定後に設計。2026-07-28）
